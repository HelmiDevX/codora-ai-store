...
export const useCouponsStore = create<CouponsStoreState>()((set, get) => ({
  ...
  fetchInitialData: async () => {
    ...
    try {
      ...
      if (liveCoupons && Object.keys(liveCoupons).length > 0) {
        ...
        set({ coupons: MOCK_COUPONS, isLoading: false });
      }
  ...
  addCoupon: async (coupon) => {
    ...
    const res = await upsertCouponToSupabase(formattedCoupon);
    if (!res.success) {
      return { success: false, error: res.error || 'فشل في حفظ الكوبون في السحابة' };
    }
  ...
  toggleCouponActive: async (code) => {
    ...
    // 1. Direct Supabase Query first
    const res = await upsertCouponToSupabase(targetCoupon);
    if (!res.success) {
      return { success: false, error: res.error || 'فشل في تغيير حالة الكوبون' };
    }
  ...
  deleteCoupon: async (code) => {
    ...
    // 1. Direct Supabase Query first
    const res = await deleteCouponFromSupabase(cleanCode);
    if (!res.success) {
      return { success: false, error: res.error || 'فشل في حذف الكوبون من السحابة' };
    }
